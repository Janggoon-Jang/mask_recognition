from PIL import Image, ImageOps, ImageDraw
import os
import json
from pathlib import Path
import matplotlib.pyplot as plt

def load_args(json_path: str = "/workspaces/mask_recognition/src/MaskRecog/preprocess/args_preprocess.json") -> dict:
    '''
    Load the args_preprocess.json and return it as a dictionary.
    '''
    with open(json_path, 'r') as f:
        return json.load(f)
    
def get_data_paths(src_dir: Path) -> list[tuple[Path, Path]]:
    '''
    Return a list of tuples [(jpg_path, txt_path), ...] for files in the given directory.
    '''
    image_dir = src_dir / "images"
    label_dir = src_dir / "labels"
    image_paths = sorted([image for image in image_dir.glob("*")])

    data_paths = []    
    for image_path in image_paths:
         label_path = label_dir / image_path.with_suffix(".txt").name
         data_paths.append((image_path, label_path))
        
    return data_paths


def load_image(image_path: Path) -> Image:
    '''
    Load image data

    Returns
        A PIL.Image instance
    '''
    try:
        image = Image.open(image_path)
    except Exception as e:
        print(f"{e}: failed to load image {image_path}")
    return image


def load_yolo_label(txt_path: Path) -> list[tuple[float, int, int, int, int]]:
    '''
    Load YOLO-format label data from a .txt file.

    Returns
        A list of tuples [(class_id, x_center, y_center, width, height), ...]
    '''
    labels = []
    with open(txt_path, 'r') as f:
        for line in f:
            parts = line.strip().split()

            if len(parts) != 5:
                raise ValueError
            cls, x, y, w, h = map(float, parts)
            labels.append((int(cls), x, y, w, h))
    return labels


class Resizer():
    def __init__(self, target_size=640):
        self.target_size = target_size
        # self.to_tensor = to_tensor

    def resize_n_adjust_labels(self, image: Image, labels: list): 
            '''
            Resize the input image to (target_size, target_size) with padding if necessary,
            and adjust the labels accordingly.

            Args:
                image_path: The input image path.
                txt_path: The input txt path.

            Returns:
                resized_img(PIL.Image): The resized and padded image.
                adjusted_labels(list of tulples): Adjusted labels after resizing.
            '''

            # Compute padding to make image square
            w, h = image.size
            max_len = max(w, h)
            diff_x = max_len - w
            diff_y = max_len - h
            xpad1 = diff_x // 2
            ypad1 = diff_y // 2

            # Apply padding and resize to target size
            padding = (xpad1, ypad1, diff_x - xpad1, diff_y - ypad1)
            image = ImageOps.expand(image, padding, fill=0)
            target_size = (self.target_size, self.target_size)
            resized_img = image.resize(target_size)

            # Adjust label coordinates to match the resized image
            adjusted_labels = []
            for line in labels:
                # Convert normalized coordinates to padded absolute values
                cls, x, y, bw, bh = line
                x_abs = x * w + xpad1
                y_abs = y * h + ypad1
                bw_abs = bw * w
                bh_abs = bh * h

                # Convert back to normalized coordinates in the resized image
                x_new = x_abs / max_len
                y_new = y_abs / max_len
                bw_new = bw_abs / max_len
                bh_new = bh_abs / max_len

                adjusted_labels.append((cls, x_new, y_new, bw_new, bh_new))

            return resized_img, adjusted_labels
    

def process_single_pair(image_path: Path, txt_path: Path, resizer) -> tuple:
        '''        
        Process a single image and label file pair.

        Args:
            image_path (str): Relative path to the input image.
            txt_path (str): Relative path to the corresponding YOLO label file.
            resizer (Resizer): An instance of the Resizer class.
            data_dir (str): Root directory containing the 'renamed' subdirectory.

        Returns:
            tuple: (resized_image, adjusted_labels, filename_without_extension)
        '''
        image = Image.open(image_path)
        labels = load_yolo_label(txt_path)
        
        processed_image, processed_label = resizer.resize_n_adjust_labels(image, labels)
        return processed_image, processed_label, image_path



def draw_bbox(image_path: Path, text_path: Path) -> None:
    img = load_image(image_path)
    labels = load_yolo_label(text_path)

    draw = ImageDraw.Draw(img)
    w, h = img.size

    for label in labels:
        cls, cx, cy, bw, bh = label

        x1 = (cx - bw / 2) * w
        y1 = (cy - bh / 2) * h
        x2 = (cx + bw / 2) * w
        y2 = (cy + bh / 2) * h

        if cls == 0: # without mask -> red
            draw.rectangle([(x1, y1), (x2, y2)], outline='red', width=5)
        if cls == 1: # with mask -> blue
            draw.rectangle([(x1, y1), (x2, y2)], outline='blue', width=5)

    plt.axis('off')
    plt.imshow(img)
    plt.show()   


def save_data(image: Image.Image, labels: list, image_path: Path, src_dir: Path):
    '''
    Save the processed image and label data using the original filename.

    Args:
        image (PIL.Image): The processed image.
        labels (list of tuples): List of (class_id, x_center, y_center, width, height).
        filename (str): Filename without extension.
        data_dir (str): Root directory where the 'processed' folder will be created.
    '''

    dst_image_dir = src_dir / "processed" / "images"
    dst_label_dir = src_dir / "processed" / "labels"

    dst_image_dir.mkdir(parents=True, exist_ok=True)
    dst_label_dir.mkdir(parents=True, exist_ok=True)

    processed_image_path = dst_image_dir / image_path.name
    processed_label_path = dst_label_dir / image_path.with_suffix(".txt").name
    
    # Save the image as a JPEG file
    image.save(processed_image_path, format='JPEG')

    # Save labels in YOLO format
    with open(processed_label_path, 'w') as f:
        for lbl in labels:
            cls, x, y, w, h = lbl
            line = f"{int(cls)} {x:.6f} {y:.6f} {w:.6f} {h:.6f}\n"
            f.write(line)